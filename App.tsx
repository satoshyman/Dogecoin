  ...
  useEffect(() => {
    ...
    usersCountRef.on('value', (snap) => {
      if (snap.exists()) {
        ...
        // Simulate a bit higher for "Scale" or use real count
    ...
    userRef.on('value', (snap) => {
      if (!snap.exists()) {
        ...
        if (referrerId && referrerId !== userId) {
          ...
          // Award fixed bonus to referrer on user join
      ...
      <div className="flex-grow overflow-y-auto pb-28">
        {activePage === 'home' ? (
          ...
          <div className="animate-in slide-in-from-right duration-300 space-y-4">
            ...
            <div className="bg-white/5 border border-white/10 rounded-[25px] p-6">
                ...
                <div className="space-y-3">
                   {[1,2,3].map(i => (
                     <div key={i} className="flex justify-between items-center text-[11px] border-b border-white/5 pb-2 last:border-0 opacity-80">
                        <div className="flex items-center gap-2">
                           <div className="w-6 h-6 bg-gold/10 rounded-full flex items-center justify-center text-[10px]">👤</div>
                           <span className="text-white font-bold">User_{Math.floor(Math.random()*9999)}</span>
                        </div>
                        <span className="text-gold font-black">+0.0001 TON</span>
                     </div>
                   ))}
                </div>
            </div>
